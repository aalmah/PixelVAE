...
with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as session:
    ...
    dx = train_data().next()[0]
    dx = np.cast[np.int32](dx)
    bn_is_training = tf.placeholder_with_default(False, shape=None, name='bn_is_training')
    bn_stats_iter = tf.placeholder_with_default(0, shape=None, name='bn_stats_iter')
    total_iters = tf.placeholder_with_default(99999, shape=None, name='total_iters')
    all_images = tf.placeholder_with_default(dx, shape=[None, N_CHANNELS, HEIGHT, WIDTH], name='all_images')
    ...
    for device_index, (device, images) in enumerate(zip(DEVICES, split_images)):
        with tf.device(device):
            ...
            def ResidualBlock(name, input_dim, output_dim, inputs, filter_size, mask_type=None, resample=None, he_init=True):
                ...
                if output_dim==input_dim and resample is None:
                    ...
                    shortcut = conv_shortcut(name+'.Shortcut', input_dim=input_dim, output_dim=output_dim,
                                             filter_size=1, mask_type=mask_type, he_init=False, biases=True, inputs=inputs)
            ...
            if NEW_COST:
                eval_images = tf.transpose(scaled_images, [0,2,3,1])
                pix_cost = discretized_mix_logistic_loss(eval_images, outputs1, BATCH_SIZE, pixel_wise=True)
            else:
                pix_cost = tf.nn.sparse_softmax_cross_entropy_with_logits(
                    tf.reshape(outputs1, [-1, 256]),
                    tf.reshape(images, [-1])
                )
                pix_cost = tf.reduce_sum(tf.reshape(pix_cost, [-1, N_CHANNELS, HEIGHT, WIDTH]), [1])

            session.run(tf.initialize_all_variables())
            # debug
            import pdb; pdb.set_trace()

            tf_pixel = tf.constant([14,14], shape=(1,2))
            tf_pixel = tf.tile(tf_pixel, [BATCH_SIZE,1])
            batch_idx = tf.concat_v2([tf.reshape(tf.range(BATCH_SIZE),[BATCH_SIZE,1]), tf_pixel], 1)
            nll_pixel = tf.gather_nd(pix_cost, batch_idx)
            nll_grad = tf.gradients(nll_pixel, scaled_images)

            import pdb; pdb.set_trace()

            sum_cost = tf.reduce_sum(pix_cost)
